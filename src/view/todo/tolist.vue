<template>
  <section class="todoapp">
    ...
    <header class="header">
      <h1>todos</h1>
      <input @keyup.enter="handlekeydown" v-model="tg" class="new-todo" placeholder="接下来该做什么" autofocus type="text" />
    </header>
    <!-- 代办事项 -->
    <section class="main">
      <input id="toggle-all" class="toggle-all" type="checkbox" @click="setalltask" />
      <label for="toggle-all">Mark all as complete</label>
      <ul class="todo-list">
        <li v-for="item in filteroption"  :key="item.id" :class="{ completed: item.completed }">
          <div class="view">
            <input type="checkbox" class="toggle" @change="completedtask(item)" :checked="item.completed" />
...
const count = computed(() => todos.value.filter(item => !item.completed).length)

const filteroption = computed(() => {

  if (visiably.value == 'all')
    return todos.value
